...
| 2026-05-24 | v3 portfolio-construction round (6 iters: pure inverse-vol, vol-targeted SSO/SHY, equity-anchored RP, 2-sleeve composite, daily-deadband variant, fast-recovery momentum) | 0 standalone champions; the lone R1 PASS (vol-targeted SSO/SHY +51.84pp) was falsified by iter004 daily-deadband variant (-81pp lag, +132pp swing) — binary-cutoff artifact | **Skipped per HANDOFF** — v3 declared SEARCH SPACE EXHAUSTED with 0 candidates worth unlocking. The 2008 plateau anchors any long-only US-equity-exposed strategy to MaxRcvry ≥ ~800d across all 3 rounds. | **N/A — no candidate** |

### Cumulative finding across v1+v2+v3 (~55 iters, 3 independent rounds)

| Round | Mechanism | Iters | R3 floor empirically |
|-------|-----------|------:|---------------------:|
| v1 | Signal-driven gating (when to be in/out) | 41 | 562d (composite) |
| v2 | Blind-design signal-driven | 9 | 800-1200d |
| v3 | Portfolio construction (how much to weight) | 6 | 1000-1679d |

Three independent search classes converged on the same finding: the R3
floor (>365d MaxRcvry) is a **property of the 2004-2022 US equity market
structure**, not a strategy defect. The 2008 GFC drawdown + 2022
dual-bear anchor any long-only US-equity-exposed strategy to MaxRcvry
≥ ~800d. R3 < 365d as currently specified appears unsatisfiable on this
engine + data + signal classes.

### Spec-lookahead failure mode triangulated (two independent disproofs)

1. **v2 Phase 4 unlock** (this row above): the v1 composite, designed
   with full data visible, lags SPY by −31.79pp on the held-out 2023+
   slice.
2. **v3 iter004**: PC-002's apparent R1 PASS (+51.84pp on monthly
   variant) was falsified by daily-deadband variant (-81pp, +132pp
   swing) — binary-cutoff artifact, not robust alpha.

**Combined finding**: apparent R1 PASS results in this engine are
systematically generated by discretization choices that don't survive
perturbation tests. This is the dominant failure mode, not strategy
logic.
...
3. Future strategy research should follow the blind-design protocol from
   ...
   applied to v1 work.
4. **Do NOT start a v4 strategy round** per QA's exhaustion verdict. The
   productive next step is a methodology audit: identify and eliminate
   binary-cutoff artifacts in the engine, specify continuous-allocation
   as canonical, re-evaluate whether the 4-gate criteria (R3 < 365d
   specifically) are well-calibrated for a market with 2008 + 2022
   plateaus. That is an operator-level decision, not an autonomous-loop
   one.
