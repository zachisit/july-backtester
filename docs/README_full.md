...
└── runs/
    └── <run_id>/                        # e.g. 2026-03-02_15-12-32 or myname_2026-03-02_15-12-32
        ...
        ├── llm_verdict.json             # Machine-readable strategy verdict (always written)
...
| `output/runs/<run_id>/llm_verdict.json` | Machine-readable strategy verdict — written on every run. Contains `beats_spy`, plain-English verdict, monthly equity curves, annual returns, and curve smoothness analysis. See [LLM Verdict](#llm-verdict) below. |
...
Every run unconditionally writes `output/runs/<run_id>/llm_verdict.json`. It is designed for LLMs, scripts, and downstream tools to evaluate strategy results without parsing terminal output or PDF tearsheets.

The terminal prints a single summary line:
```
...
```json
{
  "run_id": "2026-04-30_10-00-40",
  "generated_at": "2026-04-30T14:00:47Z",
  "period": { "start": "2004-01-01", "end": "2026-04-30" },
  "benchmarks": { "SPY": 863.27 },
  "summary": {
    "total_strategies_with_trades": 3,
    "beats_spy_count": 1,
    "beats_spy_pct": 33.3
  },
  "strategies": [ ... ]
...
| `beats_spy` | boolean | `true` if strategy return > SPY B&H return |
| `verdict` | string | `"BEATS SPY by +81684.50pp"` or `"LAGS SPY by -788.75pp"` |
| `strategy_return_pct` | float | Total portfolio return % |
| `benchmarks` | object | Per-benchmark: `bh_return_pct`, `beats`, `outperformance_pp`, `verdict` |
| `sharpe_ratio` | float | Annualised Sharpe |
| `calmar_ratio` | float | CAGR / max drawdown |
| `max_drawdown_pct` | float | Maximum equity drawdown % |
| `win_rate_pct` | float | % of trades that were profitable |
| `trades` | int | Total trade count |
| `mc_score` | float | Monte Carlo score (-999 = insufficient trades) |
| `mc_verdict` | string | `"Robust"`, `"High Tail Risk"`, `"N/A (few trades)"`, etc. |
| `wfa_verdict` | string | `"Pass"`, `"Likely Overfitted"`, or `"N/A"` |
| `wfa_rolling_verdict` | string | Rolling k-fold WFA verdict or `"N/A"` |
| `equity_curve` | object | Monthly normalized equity (start = 100) for strategy + all benchmarks |
| `annual_returns` | array | Year-by-year `strategy_pct` and benchmark `_pct` values |
| `curve_smoothness` | object | Equity curve quality analysis (see below) |
| `rank` | int | Rank within the run by strategy return |

#### Equity curve

The `equity_curve` block contains `dates` (ISO month-end strings) and parallel `strategy` / `SPY` (and any other benchmark) arrays, all normalized to 100 at the first data point. This lets an LLM or charting tool render a direct visual comparison without any further calculation.

#### Curve smoothness

The `curve_smoothness` block scores the equity curve on five independent criteria:

| Criterion | Threshold | Failure message |
|---|---|---|
| R² of log-equity vs OLS linear trend | ≥ 0.90 | `"r2: 0.84 below 0.90 threshold"` |
| Positive months % | ≥ 60% | `"positive_months: 46.8% below 60% threshold"` |
| Longest plateau (months without new equity high) | < 12 | `"plateau: 58 consecutive months without new high"` |
| Upthrust count (months > mean + 3σ) | ≤ 2 | `"upthrust: 4 outlier months detected"` |
| Worst single-month drawdown | > −10% | `"drawdown: -27.7% worst month exceeds -10% threshold"` |

**`smooth_verdict`:** `"SMOOTH"` (0 failures) · `"ACCEPTABLE"` (1 failure) · `"ROUGH"` (2+ failures)

The `smooth_notes` array lists all failures in plain English. A strategy can beat SPY massively and still score `ROUGH` — a high return with a volatile, plateau-heavy curve is a real risk signal.

`curve_smoothness` is `null` when the equity history is shorter than 12 months.

#### No config required

`llm_verdict.json` is always written. There is no config flag to enable or disable it.
